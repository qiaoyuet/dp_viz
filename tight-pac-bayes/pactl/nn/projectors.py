...
class IDModule(nn.Module):
    ...
    def __init__(self, net, projector, dimension=1000):
        ...
        self.trainable_initparams = [param for param in self.trainable_initparams]  # list (by orig net layers) of trainable parameters
        self.names = list(self.names)
        self.D = sum([param.numel() for param in self.trainable_initparams])  # dimension, total num of trainable parameters
        self.subspace_params = nn.Parameter(torch.zeros(self.d))  # empty zeros flat vec of shape dimension
    ...
    def forward(self, *args, **kwargs):
        flat_projected_params = self.P @ self.subspace_params  # @ matrix multiplication
