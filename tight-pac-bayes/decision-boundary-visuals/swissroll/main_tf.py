...
class Model:
    ...
    def build_graph(self):
        ...
        # inputs to network
        self.inputs = tf.compat.v1.placeholder(dtype=tf.float32, shape=(None, self.args.ndim), name='inputs')
        self.labels = tf.compat.v1.placeholder(dtype=tf.float32, shape=(None, self.args.nclass), name='labels')
        self.is_training = tf.compat.v1.placeholder(dtype=tf.bool)  # training mode flag
        self.lr = tf.compat.v1.placeholder(tf.float32)
        self.speccoef = tf.compat.v1.placeholder(tf.float32)

        # forward prop
        a = self.inputs
        for l, nunit in enumerate(self.args.nhidden):
            a = tf.compat.v1.layers.dense(a, nunit, use_bias=True, activation='relu')
        logits = tf.compat.v1.layers.dense(a, self.args.nclass)
        ...
        self.spec, self.projvec_op, self.projvec_corr, self.eigvec = spectral_radius(self.xent,
    ...
    def setupTF(self):
        ...
        if args.pretrain_dir is not None or args.pretrain_url is not None:
            utils.download_pretrained(logdir, pretrain_dir=args.pretrain_dir,
                                      pretrain_url=args.pretrain_url)  # download it and put in logdir
            ckpt_file = join(logdir, 'model.ckpt')
            print('Loading pretrained model from ' + ckpt_file)
            var_list = list(
                set(tf.global_variables()) - set(tf.global_variables('accum')) - set(tf.global_variables('projvec')))
    ...
    def fit(self, xtrain, ytrain, xtest, ytest):
        ...
        for epoch in range(self.args.nepoch):
            ...
            if epoch < self.args.distrstep:
                distrfrac = self.args.distrfrac
            elif epoch < self.args.distrstep2:
                distrfrac = self.args.distrfrac * 2
            else:
                distrfrac = self.args.distrfrac * 4

            # lr schedule
            if epoch < self.args.lrstep:
                lr = self.args.lr
            elif epoch < self.args.lrstep2:
                lr = self.args.lr / 10
            elif epoch < self.args.lrstep3:
                lr = self.args.lr / 100
            else:
                lr = self.args.lr / 1000

            # speccoef schedule
            speccoef = self.args.speccoef * max(0, min(1, (
                        max(0, epoch - self.args.warmupStart) / self.args.warmupPeriod) ** 2))
            ...
            for b in self.args.batchsize * np.arange(nbatch):
                xbatch = np.concatenate(
                    [xtrain[b:b + self.args.batchsize, :], xdistr[b:b + int(self.args.batchsize * distrfrac), :]])
                ybatch = np.concatenate(
                    [ytrain[b:b + self.args.batchsize, :], ydistr[b:b + int(self.args.batchsize * distrfrac), :]])

                # if epoch > 6000: self.args.speccoef = 0
                _, xent, acc_train, grad_norm, weight_norm = self.sess.run(
                    [self.train_op, self.xent, self.acc, self.grad_norm, self.weight_norm],
                    {self.inputs: xbatch,
                     self.labels: ybatch,
    ...
    def evaluate(self, xtest, ytest):
        ...
        xent, acc = self.sess.run([self.xent, self.acc],
                                  {self.inputs: xtest, self.labels: ytest, self.is_training: False})
        return xent, acc
    ...
    def plot(self, xtrain, ytrain, xtest=None, ytest=None, name='plot.jpg', plttitle='plot', index=0):
        ...
        yinfer = self.infer(xinfer)
        plot(xinfer[yinfer.ravel() == 0, 0], xinfer[yinfer.ravel() == 0, 1], '.', color=[0, 0, .5], markersize=8,
             label='class 1')
        plot(xinfer[yinfer.ravel() == 1, 0], xinfer[yinfer.ravel() == 1, 1], 'x', color=[0, 0, .5], markersize=8,
             label='class 1 error')
        ...
        yinfer = self.infer(xinfer)
        plot(xinfer[yinfer.ravel() == 1, 0], xinfer[yinfer.ravel() == 1, 1], '.', color=[.5, 0, 0], markersize=8,
             label='class 2')
        plot(xinfer[yinfer.ravel() == 0, 0], xinfer[yinfer.ravel() == 0, 1], 'x', color=[.5, 0, 0], markersize=8,
             label='class 2 error')
        xinferred, yinferred = xinfer, yinfer

        axis('image');
        title(plttitle);
        legend(loc='lower left', framealpha=.5, fontsize=10);
        axis('off')
    ...
    def wiggle(self, xdata, ydata, span=1, along='random'):
        ...
        for i, c in enumerate(cfeed):
            ...
            if exists(join(logdir, 'surface.pkl')): self.plot(xdata, ydata, name=str(i / 1000) + '.jpg',
